...
function ecl2Equ<T extends { lon: number; lat: number }>(
  ...
  const eps =
    (23.0 +
      (26 + 21.45 / 60.0) / 60.0 +
      (t * (-46.815 + t * (-0.0006 + t * 0.00181))) / 3600.0) *
    DEG;
  const coseps = Math.cos(eps);
  const sineps = Math.sin(eps);
  const sinlon = Math.sin(coor.lon);
  const ra = mod2Pi(
    Math.atan2(sinlon * coseps - Math.tan(coor.lat) * sineps, Math.cos(coor.lon)),
  );
  const dec = Math.asin(
    Math.sin(coor.lat) * coseps + Math.cos(coor.lat) * sineps * sinlon,
  );
  return { ...coor, ra, dec };
}
...
function gmstRiseSet(
  ...
  const cosArg =
    (Math.sin(h) - Math.sin(latRad) * Math.sin(coor.dec)) /
    (Math.cos(latRad) * Math.cos(coor.dec));
...
function interpolateGMST(
  ...
  return (
    (timefactor * 24.07 * gmst1 - gmst0 * (gmst2 - gmst1)) /
    (timefactor * 24.07 + gmst1 - gmst2)
  );
}
